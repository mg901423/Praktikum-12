...
{			
	...
	tVector3 normalizacija(tVector3 a)
	{
		float d=((a.x*a.x) + (a.y*a.y) + (a.z*a.z));
		float dol = sqrt(d);
		return tVector3(a.x * (1 /dol) , a.y * (1/dol) , a.z * (1/dol));
	}
	tVector3 vektorski_produkt(tVector3 a, tVector3 b)
	{
...
float skalarni_produkt(tVector3 a, tVector3 b)
{
		return a.x * b.x + a.y + b.y + a.z + b.z;
}
float dolzina_vektorja(tVector3 a)
{
		float d=((a.x*a.x) + (a.y*a.y) + (a.z*a.z));
		return sqrt(d);
}
...
tVector3 playerPosition;
tVector3 playerVector;
...
{
	 //create palnes
	pl1._Position=tVector3(0,-300,0);
	pl1._Normal=tVector3(0,1,0);
	pl2._Position=tVector3(300,0,0);
	pl2._Normal=tVector3(-1,0,0);
	pl3._Position=tVector3(-300,0,0);
	pl3._Normal=tVector3(1,0,0);
	
	pl4._Position=tVector3(0,0,300);
	pl4._Normal=tVector3(0,0,-1);
	
	pl5._Position=tVector3(0,0,-300);
	pl5._Normal=tVector3(0,0,1);
}
							  ...
							  float up_x,   float up_y,   float up_z)
{
	...
	playerPosition = tVector3(mPos.x,mPos.y+0.2,mPos.z+0.1);
}
...
{
	tVector3 vVector = mView - mPos;	
	tVector3 T1 = vVector*2;
	tVector3 T2 = vVector*3;
...
int DrawGLScene(GLvoid)									
{
	...
	*/
	...
	glutSolidSphere (0.03, 1000, 1000);

	return TRUE; 

}

int Collision_Detection(Plane plane, tVector3 position, tVector3 direction, double lamda, tVector3 pNormal)
{
	double DotProduct=skalarni_produkt(position,plane._Normal);
	double l2;
	...
	tVector3 temp = plane._Position - position;
    l2=(skalarni_produkt(plane._Normal,temp))/DotProduct;
...
void idle()
{
	...
	int test2 = Collision_Detection(pl2,mPos,vVector,rt,norm);
	if (test2==1)
			  { 
				  //sekanje
				  mPos = tVector3(0,0,1);
